...
OBS: ALL ORIGINAL CODE IS PLOTTED BELOW THE USED CODE AS "#ORIGINAL CODE: (...)"
...
#install.packages("cli") #Used to troubleshoot "devtools" that could not be loaded -> worked with restart of R!
...
lf <- list.files(path = here::here("analysis/data/DiaOutlineTraces_NewNames"), 
...
#ORIGINAL CODE: lf <- list.files("E:/DiaOutlineDataset/", pattern = "\\.txt$",full.names=TRUE)
```
...
#NOTE: in the original code it want to split "path = here::here("analysis/data/Dia Outline traces")"

lf1 <- Momocs::lf_structure(lf,  
                    split = "/", # Nick: this specifies the separator for splitting. In your case it is a /
                    trim.extension = FALSE)

#ORIGINAL CODE: lf1<-lf_structure(lf,  split = "E:/DiaOutlineDataset/", trim.extension = FALSE)
...
#Nick entered the lines below to make the levels/categories

#NICK: this is, I think, where one of the problems comes in. We need to give things types. So I am just going to fill this with 3 random values. This means that "fac" will later always be these things.
...
#ORIGINAL CODE: panel(allDiatomOutlines, fac="Type", names=TRUE)
```
...
#ORIGINAL CODE: stack(coo_center(allDiatomOutlines))
```
...
calibrate_harmonicpower_efourier(allDiatomOutlines) #Nick added _efourier

#ORIGINAL CODE: calibrate_harmonicpower(allDiatomOutlines)

#NICK: using the efourier function works now. I've interpreted the output to mean that nothing much changes after about 14 harmonics, but you need to check this.
```
...
allDiatomOutlines.f <- efourier(allDiatomOutlines, nb.h=14) #Num. of harmonics set to 14 as suggested by the calibrate harmonic power - Used to include "norm = TRUE" after the first comma

#ORIGINAL CODE: allDiatomOutlines.f <- efourier(allDiatomOutlines, nb.h=32) #Num. of harmonics set to 32
```
...
#ORIGINAL CODE: allDiatomOutlines.p <- PCA(allDiatomOutlines.f)
...
#ORIGINAL CODE: tiff("E:/Supplement_10AllPCA.tiff", height = 12, width = 17, units = 'cm', compression = "lzw", res = 500)
```
...
#ORIGINAL CODE: plot(allDiatomOutlines.p, 1, chull.filled=TRUE,stars=TRUE, title="All data PCA") #Supplement 10 in the paper.
...
#ORIGINAL CODE: plot(allDiatomOutlines.p,"Type", title="All data PCA")
```

```{r plot PCA, echo=TRUE}
#Give a new headline instead of "1"

plot(allDiatomOutlines.p, 1, chull=TRUE, 1, pos.shp = "full_axes", abbreviate.labelsgroups = TRUE, points=FALSE, labelspoints = TRUE)

#ORIGINAL CODE: plot(allDiatomOutlines.p, 1, chull=TRUE, pos.shp = "full_axes", abbreviate.labelsgroups = TRUE, points=FALSE, labelspoints = TRUE)
```
...
allDiatomOutlines.l <- Momocs::LDA(allDiatomOutlines.p, fac = "Type") # NICK: here you need to tell the function your fac. Soren: the LDA works (i.e. no warnings) if I plot "Momocs::" before the code

#ORIGINAL CODE: allDiatomOutlines.l <- LDA(allDiatomOutlines.p,1)

#summary(allDiatomOutlines.l)
```


###CODE BELOW BY Sophie Schmidt (use Momocs by ::)###

```{r, LDA by Sophie, echo=TRUE}

#allDiatomOutlines.l <-  Momocs::LDA(allDiatomOutlines.p, fac = "Type") #For some reason the LDA works better if you force it to use "Momocs::"
...
#ORIGINAL CODE: tiff("E:/Fig8AllLDA.tiff", height = 12, width = 17, units = 'cm', compression = "lzw", res = 500)
```

#Plot 1 LDA

```{r, plot LDA, echo=TRUE}
#Below comes the warning message: Error in hist.default(xy[fac == levels(fac)[1]], xlim = hist.range, ylab = NA,  : invalid number of 'breaks'

plot(allDiatomOutlines.l, chull.filled=TRUE, stars=TRUE, title="All data LDA") #Fig 8 in the paper.

#ORIGINAL CODEplot(allDiatomOutlines.l, chull.filled=TRUE, stars=TRUE, title="All data LDA") #Fig 8 in the paper.
...
#ORIGINAL CODE: plot(allDiatomOutlines.l, title="All data LDA") #Another plot version
```
...
#ORIGINAL CODE: plot(allDiatomOutlines.l, 1, chull=TRUE, pos.shp = "full_axes", abbreviate.labelsgroups = TRUE, points=FALSE, labelspoints = TRUE) #Another plot version
```
...
#ORIGINAL CODE: m<-MANOVA(allDiatomOutlines.p, 'Type')
```

```{r, echo=TRUE}
mpw<-MANOVA_PW(allDiatomOutlines.p, "Type")

#ORIGINAL CODE: mpw<-MANOVA_PW(allDiatomOutlines.p, "Type")
...
#ORIGINAL CODE: sink("E:/Manova.txt")
```

```{r, echo=TRUE}
lapply(mpw, print)

#ORIGINAL CODE: lapply(mpw, print)
```
