...
lf1 <- Momocs::lf_structure(lf,  
                    ...
                    trim.extension = FALSE) #Perhaps I have to give them better names?

lf1 <- lf1 %>% separate(V1, c("project", "type", "sort", "number")) #The command splits the column v1, but it has no use -> This command is not in the original example code!
...
levels = c("great", "medium", "horrible") #Nick entered the levels
lf2$Type <- sample(levels, size = 241, replace = TRUE) #OBS: the "size" has to be the total number of vessels (i.e. 241 in this example) #Nick entered this
lf2$Type <- as_factor(lf2$Type) # unless this is a factor, it does nothing. So it needs to be converted. Nick entered this
```
...
allDiatomOutlines.f <- efourier(allDiatomOutlines, norm = TRUE, nb.h=14) #Num. of harmonics set to 23 as suggested by the calibrate harmonic power
```
